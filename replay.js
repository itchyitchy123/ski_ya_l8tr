(function(){
  ...
  document.getElementById('importReplayButton')?.addEventListener('click',()=>{const code=prompt('Paste a friend ghost code');if(!code)return;try{const decoded=decodeURIComponent(escape(atob(code.trim()))),replay=JSON.parse(decoded);if(!replay?.events?.length)throw new Error('invalid');localStorage.setItem('alpineRushLastReplay',JSON.stringify(replay));document.getElementById('importReplayButton').textContent='GHOST IMPORTED';setTimeout(()=>document.getElementById('importReplayButton').textContent='IMPORT GHOST',1400)}catch{document.getElementById('importReplayButton').textContent='INVALID CODE';setTimeout(()=>document.getElementById('importReplayButton').textContent='IMPORT GHOST',1400)}});
