(function(){
  ...
  if(!coarse)return;
  const key=(type,keyName)=>window.dispatchEvent(new KeyboardEvent(type,{key:keyName,bubbles:true}));
  let steering='';
  ...
  function pulse(name){key('keydown',name);setTimeout(()=>key('keyup',name),180)}
  ...
  addEventListener('contextmenu',event=>{if(event.target.closest('#game'))event.preventDefault()});addEventListener('touchmove',event=>{if(event.target.closest('#game'))event.preventDefault()},{passive:false});
